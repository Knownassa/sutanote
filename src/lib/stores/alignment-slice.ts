...
export type AlignmentState = Record<never, never>;
...
type AlignmentEdge = Parameters<AlignmentActions["alignSelected"]>[0];
type AlignmentStore = AlignmentSlice & {
  nodes: CanvasNode[];
...
> = (set, get) => ({
  alignSelected: (edge) => {
    ...
    const targets: Record<AlignmentEdge, number> = {
