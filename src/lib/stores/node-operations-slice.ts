...
import type { CanvasNode, CanvasNodeData, CanvasEdge } from "../persistence/types";
...
export type NodeOperationsState = Record<never, never>;
...
type NodeOperationsStore = NodeOperationsSlice & {
  nodes: CanvasNode[];
  edges: CanvasEdge[];
...
> = (set, get) => ({
  addNode: (type, position) => {
    ...
    const pos = snapToGrid ? { x: snapValue(position.x), y: snapValue(position.y) } : position;
